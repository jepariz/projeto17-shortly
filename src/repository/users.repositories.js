...
export function getLinks(userId) {
  return connection.query(
    'SELECT u.id AS id, u.name AS name, l.id AS "linkId", l."shortUrl", l.url, l."visitCount" FROM links l JOIN users u ON l."userId" = u.id WHERE l."userId"=$1 GROUP BY u.id, u.name, l.id',
    [userId]
  );
}
...
export function getTotalVisitedSum(userId) {
  return connection.query(
    'SELECT SUM(l."visitCount") AS "visitCount" FROM links l JOIN users u ON l."userId" = u.id WHERE l."userId"=$1',
    [userId]
  );
}
